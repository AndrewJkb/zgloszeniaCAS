...
if (isset($_POST['submit'])) { // Obsłuż formularz.
	...
	if ($session_grupa = 20) {
	 list($dodajacy) = mysql_fetch_row(mysql_query("SELECT members.email from members WHERE members.id = $przypisany"));
		   $naglowki = 'Reply-to: '.$dodajacy.'<'.$dodajacy.'>'.PHP_EOL;
		   $naglowki .= 'From: '.$dodajacy.'  <'.$dodajacy.'>'.PHP_EOL;
		   $naglowki .= "MIME-Version: 1.0".PHP_EOL;
		   $naglowki .= "Content-type: text/html; charset=UTF-8".PHP_EOL; 
		   //$adres = "serwis@umcs.pl";
		   $temat = 'Dodano zgłoszenie';
		   $tresc = ' ';

list($email) = mysql_fetch_row(mysql_query("SELECT members.email FROM members WHERE members.id = 4"));

if ($temat != "" || $tresc != "" || $naglowki !="") {
...
<div class="panel-heading"><center><h4>Dodawanie</h4></center></div>
		...
		<center><button type="submit" name="submit" value="Dodaj" class="btn btn-default">Dodaj</button></center><br /><br />
		</form>
        </div>
        </div>
