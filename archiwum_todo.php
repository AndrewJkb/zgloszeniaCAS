...
$sql = "SELECT COUNT(do_zrobienia.id) FROM do_zrobienia WHERE do_zrobienia.status = 1";
...
echo '<div class="panel panel-default">
		<div class="panel-heading"><center><h4>Pomysły/Sugestie zmian - Zrealizowane</h4></center></div>
	<div class="panel-body"><br>
	 <br>
	<p><ol>';		

include "php/db_connect.php";
$sql = "SELECT do_zrobienia.tekst,members.username FROM do_zrobienia,members WHERE do_zrobienia.kto_dodal = members.id AND do_zrobienia.status = 1 Limit $limit, $onpage";
$wynik = mysql_query($sql) or die(mysql_error());
while ($wiersz = mysql_fetch_array($wynik, MYSQL_ASSOC))
{
$tekst = $wiersz['tekst'];
$user = $wiersz['username'];
echo '<li>&nbsp<b>&nbsp'.strip_tags(ucfirst($tekst)).'</b>   <span class="glyphicon glyphicon-ok icon-success"></span>
<br /><span style="font-size: 12px; font-weight: bold;">Dodał:&nbsp'.ucfirst($user).'</span></li><br />';
}
echo '</ol></p>

<h5><a href="todo.php" class="btn btn-primary">Pomysły/Sugestie zmian <span class="glyphicon glyphicon-triangle-right" aria-hidden="true"></span></a></h5>
