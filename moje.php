...
$przypisanyid = $_SESSION['user_id'];
$session_grupa = $_SESSION['grupa'];

$sql = "Select count(*) from `zgloszenia` WHERE przypisany=$przypisanyid";
...
$sql = "SELECT zgloszenia.id,zgloszenia.ticketID,zgloszenia.date_add,zgloszenia.jednostka_zglaszajaca,zgloszenia.lokalizacja,zgloszenia.temat,
		zgloszenia.osoba_kontaktowa, zgloszenia.telefon,status_zgloszen.opis_statusu,members.id as memid,members.username,grupa.nazwa as obszar 
		FROM zgloszenia,status_zgloszen,members,grupa WHERE (zgloszenia.status=status_zgloszen.id_statusu AND zgloszenia.przypisany=members.id
		AND zgloszenia.grupa = grupa.grupaid) AND zgloszenia.przypisany = $przypisanyid ORDER BY date_add DESC  Limit $limit, $onpage";
...
echo '<div class="panel panel-default">
   ...
   <table><tr><td><div class="btn-group" role="group" aria-label="...">
		...
		<span class="glyphicon glyphicon-trash"></span> Usuń</a></div><!-- koniec grupowania butonów--></td>
		
		<td><form action="przepisz.php" method=post>
      <input type="hidden" name="numer" value="'.$numer.'" class="btn btn-default">
