...
function login($username, $password, $mysqli) {
    ...
    if ($stmt = $mysqli->prepare("SELECT id, username, password, salt, grupa
				  FROM members
                                  WHERE username = ? LIMIT 1")) {
        $stmt->bind_param('s', $username);  // Bind "$useranem" to parameter.
        $stmt->execute();    // Execute the prepared query.
        $stmt->store_result();

        // get variables from result.
        $stmt->bind_result($user_id, $username, $db_password, $salt, $grupa);
        $stmt->fetch();
        ...
        if ($stmt->num_rows == 1) {
            ...
            if (checkbrute($user_id, $mysqli) == true) {
                ...
                if ($db_password == $password) {
					...
					$mysqli->query("INSERT INTO login_members(log_id, user_id, log_data)
                                    VALUES ('', '$user_id', now())");
...
function login_check($mysqli) {
    ...
    if (isset($_SESSION['user_id'], $_SESSION['username'], $_SESSION['grupa'], $_SESSION['login_string'])) {
