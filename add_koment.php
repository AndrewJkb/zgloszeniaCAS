...
include "php/db_connect.php";
		   ...
		   $sql = "INSERT INTO `komentarze_zgl` (`id_zgl_hist`, `id_zgl`, `data_zgl`, `komentarz`) VALUES (NULL, '$getid', NOW(), '$komentarz')" or die('Błąd zapytania '.mysql_error());
           $wynik = mysql_query($sql) or die('Błąd zapytania do komentarze_zgl'.mysql_error());
		   $sql1 = "INSERT INTO `historia_zmian` (log_id, user_id, id_zgl, log_data, log_typ_zmiany) VALUES (NULL, '$user', '$getid',NOW(),'Dodano komentarz')";
		   ...
		   $temat = 'Dodano komentarz do zgłoszenia nr '.$getid.'';
		   $tresc = 'Użytkownik <b>'.ucfirst($imie_zmieniajacego).'</b> dodał komentarz do twojego zgłoszenia nr '.$getid.'<br><br>
			https://zgloszenia.lubman.umcs.pl/zobacz.php?id='.$getid.'
...
Niniejsza wiadomość elektroniczna lub jej załączniki mogą zawierać poufne lub chronione prawem informacje, które są przeznaczone wyłącznie dla wskazanego w nich adresata. Jeżeli nie jesteś adresatem wiadomości, prosimy o jej nieujawnianie, zawiadomienie nadawcy o jej otrzymaniu oraz usunięcie wraz z wszystkimi załącznikami. Dziękujemy.
';
